        ...
        try:
            ...
            if os.path.exists(settings.treatment_data_path):
                with open(settings.treatment_data_path, 'r', encoding='utf-8') as f:
                    reader = csv.DictReader(f)
                    for row in reader:
                        # Handle the treatment CSV format
                        diagnosis_code_name = row.get('Diagnosis Code and Name', '') or row.get('diagnosis code and name', '')
                        prescription_en = row.get('Prescription', '') or row.get('prescription', '')

                        # Find Thai prescription name from the row values
                        prescription_th = ''
                        columns = list(row.keys())
                        if len(columns) > 3:  # Should have Thai prescription column
                            for i, col in enumerate(columns):
                                if i > 2 and row.get(col):  # Skip first few columns
                                    prescription_th = row.get(col)
                                    break
...
💬 **หมายเหตุ**: ระบบจะแจ้งเตือนเมื่อแพทย์ตอบกลับแล้ว"""
    ...
    def _retrieve_medications_from_rag(self, condition: str, symptoms: List[str]) -> List[Dict]:
        ...
        for treatment in self.treatments:
            ...
            if condition in treatment_desc or any(symptom.lower() in treatment_desc for symptom in symptoms):
        ...
        symptom_diagnosis_map = {
            "common_cold": {
                "keywords": [
                    ...
                    "จาม", "จามบ่อย", "ตาแดง", "ตาคัน", "ปวดหัวเล็กน้อย",
                    ...
                    "green mucus", "yellow mucus", "cold", "common cold"
                ],
            ...
            "gastritis": {
                "keywords": [
                    # Thai comprehensive
                    "ปวดท้อง", "ปวดลิ้นปี่", "แสบร้อน", "แสบกลางอก", "คลื่นไส้", "อาเจียน",
                    "ท้องอืด", "ท้องเฟ้อ", "เรอบ่อย", "กรดไหลย้อน", "กินไม่ได้", "หิวแต่กินไม่ลง",
                    "อาหารไม่ย่อย", "ปวดหลังอาหาร", "ปวดท้องว่าง",
                    # English
                    "stomach pain", "gastric pain", "heartburn", "nausea", "vomiting", "bloating",
                    "acid reflux", "indigestion", "loss of appetite", "epigastric pain"
                ],
                "icd_code": "K29.70", "thai_name": "กระเพาะอักเสบ", "english_name": "Gastritis", "confidence_base": 70,
...
class TreatmentAgent(BaseAgent):
    ...
    async def _recommend_medications(self, diagnosis: Optional[Dict], patient_info: Optional[PatientInfo]) -> List[Dict]:
        ...
        # Extract English name properly - handle cases like "ไข้หวัดใหญ่ (Influenza)"
        english_name = diagnosis.get("english_name", "")
        if "(" in english_name and ")" in english_name:
            # Extract text in parentheses as the English name
            condition = english_name.split("(")[1].split(")")[0].lower()
        else:
...
Warnings: ปรึกษาแพทย์หากอาการไม่ดีขึ้นภายใน 3 วัน
    ...
    def _get_lifestyle_recommendations(self, diagnosis: Optional[Dict]) -> List[str]:
        ...
        if "cold" in condition or "flu" in condition or "fever" in condition:
            specific_recommendations = [
                "หลีกเลี่ยงอากาศเย็นและฝุ่นควัน สวมผ้าปิดปากเมื่อออกจากบ้าน",
                "กินอาหารอุ่นๆ ซุปไก่ และผลไม้ที่มีวิตามินซี เช่น ส้ม มะนาว",
            ...
            specific_recommendations = [
                "หลีกเลี่ยงอาหารเผ็ด เปรี้ยว มัน และของดอง",
                "รับประทานอาหารเป็นเวลาสม่ำเสมอ ครั้งละน้อยแต่บ่อยครั้ง (5-6 มื้อเล็กๆ)",
                "หลีกเลี่ยงกาแฟ ชา เครื่องดื่มแอลกอฮอล์ และสารกระตุ้น",
                "เคี้ยวอาหารให้ละเอียด รับประทานอย่างช้าๆ",
                "หลีกเลี่ยงการกินอาหารก่อนนอน 2-3 ชั่วโมง"
            ]
