    ...
    async def _load_medical_knowledge(self):
        ...
        if os.path.exists(settings.diagnosis_data_path):
            with open(settings.diagnosis_data_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    diagnosis_code_name = row.get('Diagnosis Code and Name', '')
                    thai_name = list(row.values())[-1] if len(row) > 1 else ''
                    ...
                    frequency, severity, urgency = self._categorize_condition(english_name, icd_code)

                    # Generate symptoms and keywords
                    symptoms, keywords = self._generate_symptoms_keywords(english_name, thai_name)
        ...
        if os.path.exists(settings.treatment_data_path):
            with open(settings.treatment_data_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    diagnosis_name = row.get('Diagnosis Code and Name', '')
                    treatment_name = row.get('Prescription', '')

                    # Find matching diagnosis in knowledge base
                    for item in self.knowledge_base:
                        if diagnosis_name and diagnosis_name.lower() in item.name_en.lower():
                            if treatment_name and treatment_name not in item.treatments:
                                item.treatments.append(treatment_name)
