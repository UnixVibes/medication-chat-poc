...
export default function ChatInterface({ onSendMessage, initialMessages = [], roomId }: ChatInterfaceProps) {
  ...
  // Polling for real-time updates instead of WebSocket
  useEffect(() => {
    if (!roomId) return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (chat && chat.messages.length > messages.length) {
            ...
            const formattedMessages: Message[] = chat.messages.map((msg: any, index: number) => ({
              id: msg.id || `msg-${index}`,
              content: msg.content,
              role: msg.role === 'patient' ? 'user' : msg.role === 'doctor' ? 'assistant' : msg.role,
              timestamp: new Date(msg.timestamp || Date.now())
            }));
            setMessages(formattedMessages);
          }
  ...
  const handleSendMessage = async () => {
    ...
    // Send message via WebSocket for real-time sync
    ...
    try {
      ...
      // Send AI response via WebSocket for real-time sync
